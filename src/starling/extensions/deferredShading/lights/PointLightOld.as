package starling.extensions.deferredShading.lights
{
     ...
     */
    public class PointLightOld extends Light implements IShadowMappedLight, IAreaLight
    {
        ...
        private static const POSITION_ATTRIBUTE:String = 'position';
        ...
        public function PointLightOld(color:uint = 0xFFFFFF, strength:Number = 1.0, radius:Number = 50, attenuation:Number = 15)
        {
            var vertexData:VertexData = new VertexData(VertexDataFormat.fromString(POSITION_ATTRIBUTE + ':float2'), mNumEdges + 1);
            var indexData:IndexData = new IndexData(mNumEdges * 3);
        ...
        public override function getBounds(targetSpace:DisplayObject, out:Rectangle = null):Rectangle
        {
            ...
            return vertexData.getBounds(POSITION_ATTRIBUTE, transformationMatrix, 0, -1, out);
        }
        ...
        private function setupVertices():void
        {
            ...
            for(i = 0; i < mNumEdges; ++i)
            {
                ...
                vertexData.setPoint(i, POSITION_ATTRIBUTE, edge.x, edge.y);
            }

            // Center vertex
            vertexData.setPoint(mNumEdges, POSITION_ATTRIBUTE, 0.0, 0.0);
            ...
            vertexBuffer = context.createVertexBuffer(vertexData.numVertices, 2);
            vertexBuffer.uploadFromByteArray(vertexData.rawData, 0, 0, vertexData.numVertices);

            indexBuffer = context.createIndexBuffer(indexData.numIndices);
            indexBuffer.uploadFromByteArray(indexData.rawData, 0, 0, indexData.numIndices);
        ...
        {
            if(DeferredShadingContainer.renderPass != RenderPass.LIGHTS) return;
            ...
            Starling.painter.getProgram(_castsShadows ? POINT_LIGHT_PROGRAM_WITH_SHADOWS : POINT_LIGHT_PROGRAM).activate(context);
            context.setVertexBufferAt(0, vertexBuffer, 0, Context3DVertexBufferFormat.FLOAT_2);
//            vertexFormat.setVertexBufferAttribute(vertexBuffer, 0, POSITION_ATTRIBUTE);
         ...
         */
        public function renderShadowMap(support:Painter,
                                        occluders:Texture,
                                        ...
                                        indexBuffer:IndexBuffer3D):void
        {
            ...
            support.getProgram(SHADOWMAP_PROGRAM).activate(context);
        ...
        {
            var target:Painter = Starling.current.painter;
            ...
            target.registerProgram(POINT_LIGHT_PROGRAM, new Program(vertexProgramAssembler.agalcode, fragmentProgramAssembler.agalcode));
            ...
            target.registerProgram(POINT_LIGHT_PROGRAM_WITH_SHADOWS, new Program(vertexProgramAssembler.agalcode, fragmentProgramAssembler.agalcode));
